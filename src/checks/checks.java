...
public class checks {
    ...
    public static void chechRule14() {
    }  //dar listenner tarif shod enterExpr

    public static void chechRule15() {
    }  //dar listenner tarif shod enterExpr

    public static void chechRule16() {
    }  //dar listenner tarif shod enterExpr

    public static void chechRule17() {
    }  //dar listenner tarif shod var_def & assign

    public static void chechRule21() {
    }  //dar listenner tarif exit_program


    //utility
    public static Boolean checkingPrimitiveType1(lulu2_grammerParser.ExprContext expr, scopeClass current) {
        if (expr.getChildCount() == 1) {
            checkingPrimitiveType(current, expr.var().ref(0).ID().getText());
            return true;
        } else {
            if (checkingPrimitiveType1(expr.expr(0), current) == null) return false;
            if (checkingPrimitiveType1(expr.expr(1), current) == null) return false;
        }
        return true;
    }
    ...
    public static Types checkingPrimitiveType(scopeClass current, String id) {
        if (current.getSymbolTable().get(id) != null) {
            if (current.getSymbolTable().get(id).getType() == Types.STRING ||
                    ...
                    current.getSymbolTable().get(id).getType() == Types.BOOL)
    ...
    public static Types findExprType(lulu2_grammerParser.ExprContext expr, scopeClass current) {
        if (expr.getChildCount() == 1) {
            if (expr.const_val() == null)
                ...
                if (expr.const_val().INT_CONST() != null) return Types.INT;
                else if (expr.const_val().BOOL_CONST() != null) return Types.BOOL;
                else if (expr.const_val().STRING_CONST() != null) return Types.STRING;
